...
export default function Contacts() {
  ...
  const { friendRequests } = useSelector((state: any) => state.contact);
  ...
  return (
    <Card className='flex-row items-center gap-5 p-6 overflow-hidden'>
      <Avatar alt='Avatar'>
        ...
        <AvatarFallback>
          <Text>{contact?.senderUsername?.charAt(0)}</Text>
        </AvatarFallback>
      </Avatar>
      <View className='flex-1 overflow-hidden'>
        <View className='flex-row justify-between items-center'>
          <Text numberOfLines={2} className='text-lg font-semibold flex-1 mr-2'>
            {contact?.senderUsername}
          </Text>
          <View className='w-28 flex-row justify-end items-center gap-2'>
            <TouchableOpacity
              activeOpacity={0.8}
              className='bg-green-600 rounded-full p-2 w-12 h-12 items-center justify-center'
              onPress={() => console.log('Tick button pressed')}
            >
